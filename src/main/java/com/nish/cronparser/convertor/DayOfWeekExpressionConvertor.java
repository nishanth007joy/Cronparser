...
public class DayOfWeekExpressionConvertor implements IConvertor {
	...
	public Optional<List<String>> convertExpression(String indivudualExpression) {
		if (CharMatcher.anyOf(CRONTypes.ASTERISK.getTypeSymbol()).matchesAllOf(indivudualExpression)) {
			...
			return Optional.of(IntStream.rangeClosed(DayOfWeek.MONDAY.getValue(), DayOfWeek.SUNDAY.getValue())
			...
			List<String> week = Splitter.on(CRONTypes.COMMA.getTypeSymbol()).splitToList(indivudualExpression);
			if (!validateExpression.validateWeeks(week)) {
			...
			if (!validateExpression.validateWeekRange(range.get(0), range.get(1))) {
				throw new ParseException(ParseException.ErrorCode.NOT_VALID_EXPRESSION);
			}
